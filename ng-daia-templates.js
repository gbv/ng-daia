angular.module('ngDAIA').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('template/daia-response.html',
    "<div class=\"daia-result\"><div ng-if=\"daia.institution.content.length\"><span class=\"daia-label\" translate=\"INST\">Queried institution:</span> <a ng-if=\"daia.institution.href.length\" href=\"{{daia.institution.href}}\">{{daia.institution.content}}</a></div><div ng-if=\"daia.document[0].href.length\"><a translate=\"CAT\" href=\"{{daia.document[0].href}}\">Catalogue entry</a></div><div><span ng-if=\"!daia.document.length\" translate=\"NR\">no records found</span></div><div ng-if=\"daia.document.length\" daia-documents=\"daia.document\"><div class=\"daia-document\" ng-repeat=\"i in daia.document[0].item\"><div daia-item=\"i\"></div></div></div></div>"
  );


  $templateCache.put('template/daia-simple.html',
    "<span class=\"daia-label\" translate=\"STATUS\">current status:</span> <span ng-if=\"status == 'openaccess'\" class=\"availability availability-available\" translate=\"{{status}}\">{{status}}</span> <span ng-if=\"status == 'loan'\" class=\"availability availability-available\" translate=\"{{status}}\">{{status}}</span> <span ng-if=\"status == 'presentation'\" class=\"availability availability-presentation\" translate=\"{{status}}\">{{status}}</span> <span ng-if=\"status != 'openaccess' && status != 'loan' && status != 'presentation' && status != 'expected'\" class=\"availability availability-unavailable\" translate=\"{{status}}\">{{status}}</span> <span ng-if=\"expected\" class=\"availability availability-unavailable\" translate=\"EXP\">until</span> <span ng-if=\"expected\" class=\"availability availability-unavailable\">{{expected}}</span>"
  );
